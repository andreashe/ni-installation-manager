 ...
 *
 * All key paths are HKLM-relative (e.g. `SOFTWARE\WOW6432Node\Native Instruments\Kontakt 8`)
 * because every NI product key lives under HKEY_LOCAL_MACHINE.
 */
export class RegistryService implements RegistryMutationBackend {
  constructor(private readonly logger: LoggerService) {}

  /**
   * List the names of all direct subkeys of an HKLM-relative key.
   * Returns an empty array when the key does not exist (e.g. 32-bit root
   * missing on some systems).
   */
  listSubkeyNames(keyPath: string): string[] {
    const key = reg.openKey(reg.HKLM, keyPath, reg.Access.READ);
    if (!key) {
      this.logger.debug(`Key not found: HKLM\\${keyPath}`, LOG_SOURCE);
      return [];
    }
    try {
  ...
  /** True when the HKLM-relative key exists. */
  ...
  /**
   * Read all values of an HKLM-relative key as JSON-serializable DTOs
  ...
  /**
   * Convenience: read one string value (e.g. `ContentDir`) from an
   * HKLM-relative key. Null when key or value is missing or not a string.
   */
  readStringValue(keyPath: string, valueName: string): string | null {
    const parsed = reg.getValue(reg.HKLM, keyPath, valueName);
    return typeof parsed === 'string' ? parsed : null;
  }

  // ── RegistryMutationBackend (only called through RegistryGuard) ────────

  /** Recursively delete an HKLM-relative key and everything below it. */
  async deleteKeyTree(keyPath: string): Promise<void> {
    reg.deleteTree(reg.HKLM, keyPath);
    // deleteTree removes children; the key itself needs a separate delete.
    reg.deleteKey(reg.HKLM, keyPath);
  }

  /** Delete a single value inside an HKLM-relative key. */
  async deleteValue(keyPath: string, valueName: string): Promise<void> {
    reg.deleteKeyValue(reg.HKLM, keyPath, valueName);
  }

  /**
   * Restore all values of one backed-up key (TODO8): creates the
   * HKLM-relative key (incl. missing ancestors — RegCreateKeyEx semantics)
   * and writes every value back with its original registry type. Inverse of
  ...
  async restoreKeyValues(keyPath: string, values: RegistryValueDto[]): Promise<void> {
    const key = reg.createKey(reg.HKLM, keyPath, reg.Access.ALL_ACCESS);
    try {
      for (const value of values) {
        if (value.value === null) {
          this.logger.warn(
            `Skipping registry value HKLM\\${keyPath}\\${value.name} — no data in backup`,
            LOG_SOURCE,
          );
          continue;
        }
